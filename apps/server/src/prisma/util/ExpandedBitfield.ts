...
export function getAllBitfieldCombinations<T extends number>(enumType: Record<string, T | string>, bitfield: T): number[] {
  const allCombinations = ExpandedBitwiseEnumArray.fromTSBitwiseEnum(enumType);
  return allCombinations.and(bitfield);
}
